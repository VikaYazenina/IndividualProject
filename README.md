...
Веб-сервис персональной агрегации контента: следит за выбранными источниками
(RSS-ленты и обычные страницы без RSS), сохраняет новые материалы в архив
и раз в сутки присылает email-дайджест. Есть веб-интерфейс, REST API,
поиск/фильтрация, экспорт в CSV и базовая аналитика потребления.

Целевая аудитория, проблемное поле и метрики эффективности — в `docs/proposal.md`.
...
- [x] Этап 1 — проектирование структуры БД
- [x] Этап 2 — модуль парсинга RSS, функция сбора новых материалов
- [x] Этап 3 — REST API управления источниками, парсинг HTML-страниц без RSS
- [x] Этап 4 — планировщик (APScheduler) + email-рассылка (Jinja2 + SMTP)
- [x] Этап 5 — веб-интерфейс: страница управления источниками
- [x] Этап 6 — веб-интерфейс: архив, отложенное чтение, поиск и фильтрация
- [x] Этап 7 — тесты (pytest, 9 тестов), экспорт в CSV, Procfile для деплоя

## Стек

Python, FastAPI, SQLAlchemy, feedparser, BeautifulSoup, APScheduler, Jinja2, SMTP, pytest.
